        ...
        <nav class="hidden md:flex space-x-4 items-center">
            @php
                $menuItems = [
                    ...
                    'appellants.index' => 'Appellant',
                    'respondents.index' => 'Respondent',
                    'advocates.index' => 'Advocates',
                    'mediators.index' => 'Mediators',
                    'courts.index' => 'Court',
                    'subjects.index' => 'Subject',
                    'issues.index' => 'Issue',
                    'statutes.index' => 'Statute',
                    'users.index' => 'Users'
                ];
            @endphp

            @foreach ($menuItems as $route => $label)
                <a href="{{ route($route) }}" class="p-2 rounded-lg {{ request()->routeIs(str_replace('.index', '.*', $route)) ? 'bg-blue-700 text-white' : 'hover:bg-blue-700 hover:text-white' }}">
                    {{ $label }}
                </a>
            @endforeach
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm self-center">
                    LOG OUT
                </button>
            </form>
        </nav>
...
<main class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Advocates</h1>
        <button onclick="openAdvocateModal()" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-500">
            + Add Advocate
        </button>
    </div>
        ...
        <table id="recentCasesTable" class="min-w-full border mt-4 bg-white rounded shadow ">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-2 border">Bar Number</th>
            ...
            <tbody>
                @foreach($advocates as $advocate)
                    <tr class="hover:bg-gray-100">
                        <td class="px-4 py-2 border">{{ $advocate->id }}</td>
                        <td class="px-4 py-2 border">{{ $advocate->name }}</td>
                        <td class="px-4 py-2 border">{{ $advocate->bar_number }}</td>
                        <td class="px-4 py-2 border">{{ $advocate->emailId ?? 'N/A' }}</td>
                        <td class="px-4 py-2 border space-x-2">
                            <a href="{{ route('advocates.show', $advocate->id) }}" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">View</a>
                            <a href="{{ route('advocates.edit', $advocate->id) }}" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</a>
                            <form action="{{ route('advocates.destroy', $advocate->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this advocate?');">
                                @csrf
                                @method('DELETE')
