...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <header class="bg-white shadow-md w-full">
        <div class="container mx-auto flex justify-between items-center px-6 py-3">
            ...
            <nav class="hidden md:flex space-x-4 items-center">
                ...
                <a href="{{ route('advocates.index') }}" class="bg-green-700 text-white px-3 py-1 rounded hover:bg-green-900 text-sm self-center">Advocates</a>
            </nav>
        </div>
    </header>
    ...
    <main class="container mx-auto px-4 py-6 flex-grow max-w-4xl">
        <h1 class="text-xl font-bold mb-6">Advocate Details</h1>

        <div class="bg-white p-6 rounded-lg shadow-lg text-gray-800 space-y-2 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4">
            <div><strong>Name:</strong> {{ $advocate->name }}</div>
            <div><strong>Bar Number:</strong> {{ $advocate->bar_number }}</div>
            <div><strong>Email ID:</strong> {{ $advocate->emailId ?? 'N/A' }}</div>
            <div><strong>Mobile:</strong> {{ $advocate->mobile ?? 'N/A' }}</div>
        </div>
