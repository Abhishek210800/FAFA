...
<main class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('courts.create') }}" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-500">
            + Add Court
        </a>
    </div>
    ...
    <div id="datatableContainer" class="opacity-0 invisible transition-opacity duration-500">
        <table id="recentCasesTable" class="min-w-full border mt-4 bg-white rounded shadow ">
            <thead class="bg-gray-200">
                <tr>
                    <th class="px-4 py-2 border">Court Code</th>
                    <th class="px-4 py-2 border">Court Name</th>
                    <th class="px-4 py-2 border">Count Code</th>
            ...
            <tbody>
                @foreach($courts as $court)
                    <tr class="hover:bg-gray-100">
                        <td class="px-4 py-2 border">{{ str_pad($court->AG_Courtcode, 3, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-4 py-2 border">{{ $court->Court_Name }}</td>
                        <td class="px-4 py-2 border">{{ $court->Count_code }}</td>
                        <td class="px-4 py-2 border space-x-2">
                            <a href="{{ route('courts.show', str_pad($court->AG_Courtcode, 3, '0', STR_PAD_LEFT)) }}" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">View</a>
                            <a href="{{ route('courts.edit', str_pad($court->AG_Courtcode, 3, '0', STR_PAD_LEFT)) }}" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</a>
                            <form action="{{ route('courts.destroy', str_pad($court->AG_Courtcode, 3, '0', STR_PAD_LEFT)) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this court?');">
                                @csrf
                                @method('DELETE')
