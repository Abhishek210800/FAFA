...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <main class="container mx-auto px-4 py-6 flex-grow">
        <h1 class="text-xl font-bold mb-4">Edit Mediator</h1>

        <form action="{{ route('mediators.update', $mediator->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $mediator->name) }}" class="p-3 border border-gray-300 rounded-lg w-full">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Address</label>
                        <input type="text" name="address" value="{{ old('address', $mediator->address) }}" class="p-3 border border-gray-300 rounded-lg w-full">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="email" name="emailId" value="{{ old('emailId', $mediator->emailId) }}" class="p-3 border border-gray-300 rounded-lg w-full">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Mobile</label>
                        <input type="text" name="mobile" value="{{ old('mobile', $mediator->mobile) }}" class="p-3 border border-gray-300 rounded-lg w-full">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                      <label class="block text-gray-700 font-semibold text-sm">Qualification</label>
                      <input type="text" name="qualification" value="{{ old('qualification', $mediator->qualification) }}" class="p-3 border border-gray-300 rounded-lg w-full">
                  </div>
              </div>

                <div class="flex justify-end space-x-4">
                    <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Update</button>
                    <a href="{{ route('mediators.index') }}" class="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500">Cancel</a>
                </div>
            </div>
        </form>
    </main>
