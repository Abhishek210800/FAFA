...
<header class="bg-white shadow-md w-full">
    <div class="container mx-auto flex justify-between items-center px-6 py-3">
        ...
        <nav class="hidden md:flex space-x-4 items-center">
            ...
            <a href="{{ route('appellants.index') }}" class="text-black hover:text-blue-500">Appellant</a>						                                                                                <a href="{{ route('respondents.index') }}" class="text-black hover:text-blue-500">Respondent</a>
...
<main class="container mx-auto px-4 py-6">
    <h1 class="text-xl font-bold mb-4">Edit Respondent</h1>
            ...
            <fieldset id="complainant-accordion-content" class="p-6 bg-white space-y-8 rounded-b-lg shadow-sm transition-all duration-500">
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $respondent->name) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Father's Name</label>
                        <input type="text" name="father" value="{{ old('father', $respondent->father) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Date of Birth</label>
                        <input type="date" name="dob" value="{{ old('dob', $respondent->dob) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-5 gap-6">
                    <div>
                        ...
                        <input type="text" name="address" value="{{ old('address', $respondent->address) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                        ...
                        <select name="city_id" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                            ...
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}" {{ old('city_id', $respondent->city_id) == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">District</label>
                        <input type="text" name="district" value="{{ old('district', $respondent->district) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                        ...
                        <select name="state_id" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" {{ old('state_id', $respondent->state_id) == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Pincode</label>
                        <input type="text" name="pincode" value="{{ old('pincode', $respondent->pincode) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="tel" name="mobile" pattern="[0-9]{10}" value="{{ old('mobile', $respondent->mobile) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                    <div>
                        <label class="block text-gray-700 font-semibold text-sm">Email</label>
                        <input type="email" name="email" value="{{ old('email', $respondent->email) }}" class="p-3 border border-gray-300 bg-white rounded-lg w-full">
                    </div>
                </div>
                ...
                <div class="mt-6 flex justify-end space-x-4">
                    ...
                    <a href="{{ route('respondents.index') }}" class="bg-gray-400 text-white px-4 py-2 rounded-lg hover:bg-gray-500">Cancel</a>
                </div>
            </fieldset>
        </div>
    </form>
