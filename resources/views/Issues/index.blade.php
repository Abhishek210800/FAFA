    ...
    <div id="datatableContainer" class="opacity-0 invisible transition-opacity duration-500">
            <table id="recentCasesTable" class="min-w-full border mt-4 ">
                <thead class="bg-gray-200">
                    <tr>
                        <th class="px-4 py-2 border">Issue Code</th>
                        <th class="px-4 py-2 border">Issue Name</th>
                ...
                <tbody>
                    @foreach($issues as $issue)
                        <tr class="hover:bg-gray-100">
                            <td class="px-4 py-2 border">{{ $issue->Issue_Code }}</td>
                            <td class="px-4 py-2 border">{{ $issue->IssueName }}</td>
                            <td class="px-4 py-2 border space-x-2">
                                <a href="{{ route('issues.show', $issue->Issue_Code) }}" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">View</a>

                                <a href="{{ route('issues.edit', $issue->Issue_Code) }}" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</a>
                                
                                <form action="{{ route('issues.destroy', $issue->Issue_Code) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this subject?');">
                                    @csrf
                                    @method('DELETE')
