...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <main class="container mx-auto px-4 py-6 flex-grow">
        ...
        <form action="{{ route('subjects.update', $subject->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-lg space-y-6">
            @csrf
            @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-gray-700 font-semibold text-sm mb-1">Subject ID</label>
                    <input type="text" name="subject_id" value="{{ old('subject_id', $subject->subject_id) }}"
