    ...
    <table id="recentCasesTable" class="min-w-full border mt-4 bg-white rounded shadow ">
        <thead class="bg-gray-200">
            <tr>
                ...
                <th class="px-4 py-2 border">Subject ID</th>
        ...
        <tbody>
            @foreach($subjects as $subject)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="px-4 py-2 border">{{ $subject->subject_id }}</td>
                    <td class="px-4 py-2 border">{{ $subject->name }}</td>
                    <td class="px-4 py-2 border space-x-2">
                        <a href="{{ route('subjects.show', $subject->subject_id) }}" class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">View</a>
                        <a href="{{ route('subjects.edit', $subject->subject_id) }}" class="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</a>
                        <form action="{{ route('subjects.destroy', $subject->subject_id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this subject?');">
                            @csrf
                            @method('DELETE')
